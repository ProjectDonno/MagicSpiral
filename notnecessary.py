# Первый вариант программы, до изменённого алгоритма спиральки.
it_works = True

def magicspiral(mas, N):
    for i in range(1, N + 1):
        print(f"{i}", end=" ")
    print()

    secondrow = str(N + (N - 1) + (N - 1) + (N - 2))
    for i in range(N - 1):
        print(f"{str(int(secondrow) + i)}", end=" ")
    print(f"{str(int(N + 1))}", end=" ")

    secondrow = int(secondrow) + (N - 2) + 1

def writearray(string):
...
# Создание, заполнение и вывод двумерного массива.
a = 5
b = 5
r = 1
mas = []
...
# Создание, заполнение и вывод магической спирали.
def magicspiral(N):
